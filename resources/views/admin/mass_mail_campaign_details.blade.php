                    ...
                    <div class="block-content block-content-full">
                        <div class="text-center">
                            ...
                            <div class="text-muted">
                                ...
                                {{-- <small>({{ $campaign->total_recipients > 0 ?
                ...
                <div class="row mt-4">
                    ...
                    <div class="col-md-4">
                        <div class="block block-rounded">
                            <div class="block-content block-content-full">
                                <div class="text-center">
                                    <div class="fs-2 fw-bold">{{ number_format($smsTotalSent) }}</div>
                                    <div class="text-muted">SMS Sent</div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="block block-rounded">
                            <div class="block-content block-content-full">
                                <div class="text-center">
                                    <div class="fs-2 fw-bold text-success">{{ number_format($smsDelivered) }}</div>
                                    <div class="text-muted">
                                        Delivered
                                        <small>({{ $smsTotalSent > 0 ? round(($smsDelivered / $smsTotalSent) * 100, 1) : 0 }}%)</small>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="col-md-4">
                        <div class="block block-rounded">
                            <div class="block-content block-content-full">
                                <div class="text-center">
                                    <div class="fs-2 fw-bold text-danger">{{ number_format($smsFailed) }}</div>
            ...
            <div class="block-content">
                ...
                <div>
                    ...
                    <div class="border p-3 mt-2" style="max-height: 200px; overflow-y: auto;">
                        {!! $campaign->channel === 'email' ? $campaign->message : $campaign->sms_message !!}
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="block-content">
                <table class="table table-hover">
                    ...
                    <tbody>
                        @forelse($logs as $log)
                            <tr>
                                <td>{{ $log->user->name ?? 'N/A' }}
                                    <span class="badge bg-{{ $log->channel === 'email' ? 'primary' : 'info' }}">
                                        {{ strtoupper($log->channel) }}
                                    </span>
                                </td>
                                <td>{{ $log->channel === 'email' ? $log->email : $log->phone }}</td>
                                <td>{{ $log->email }}</td>
                                <td>
                                    @php
                                        $statusColors = [
