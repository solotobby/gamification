    ...
    <div class="bg-body-light">
        <div class="content content-full">
            <div class="d-flex flex-column flex-sm-row justify-content-sm-between align-items-sm-center">
                <h1 class="flex-grow-1 fs-3 fw-semibold my-2 my-sm-3">USD Verified Users</h1>
                <nav class="flex-shrink-0 my-2 my-sm-0 ms-sm-3" aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item">Users</li>
                        <li class="breadcrumb-item active" aria-current="page">USD Users List</li>
                    </ol>
                </nav>
            </div>
        </div>
    ...
    <div class="content">
        ...
        <div class="block block-rounded">
            <div class="block-header block-header-default">
                <h3 class="block-title">USD Verified Users List - {{ $verifiedUsers->total() }}</h3>
                        ...
                        {{-- <table class="table table-bordered table-striped table-vcenter js-dataTable-buttons"> --}}
                            ...
                            <tbody>
                                ...
                                @foreach ($verifiedUsers as $user)
                                    <tr>
                                        <th scope="row">{{ $i++ }}.</th>
                                        <td class="fw-semibold"><a href="{{ url('user/' . $user->user->id . '/info') }}"
                                                target="_blank"> {{$user->user->name }}</a></td>
                                        <td>{{ $user->user->email }}</td>
                                        <td>{{ $user->user->phone }}</td>
                                        {{-- <td>&#8358;{{ number_format(@$user->wallet->balance) }}</td> --}}
                                        {{-- <td>{{ @$user->referees->count() }}</td> --}}
                                        {{-- <td>{{ $user->is_verified == "1" ? 'Verified' : 'unverified' }}</td> --}}
                                        <td>{{ $user->user->source }}</td>
                                        <td>
                                            {{ $user->user->verified_at ? \Carbon\Carbon::parse($user->user->verified_at)->format('d/m/Y @ h:i:s a') : '' }}
                                        </td>
                                        <td>{{ \Carbon\Carbon::parse($user->user->created_at)->format('d/m/Y @ h:i:s a') }}</td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
