              ...
              @if(auth()->user()->wallet->base_currency == 'Naira')
                    ...
                    @if(auth()->user()->is_verified)
                            <p> You are verified for naira jobs but not elligible for dollar jobs. 
                                ...
                                <p>A dollar wallet verification fee is $5 (&#8358;{{ dollar_naira() * 5 }}). Since you are Naira verified, you are qualified for our 20% discount!</p>
                            <a href="{{ url('upgrade/part') }}" class="btn btn-primary"> <li class="fa fa-link"> </li> Get Verified for Dollar Jobs! </a>
                    @else
                            <p> You are not verified for dollar jobs. Please click the button below to get a verified dollar wallet!</p>
                            <p> A dollar wallet verification fee is $5 (&#8358;{{ dollar_naira() * 5 }}). </p>
                            <a href="{{ url('upgrade/full') }}" class="btn btn-primary"> <li class="fa fa-link"> </li> Get Verified for Dollar Jobs! </a>
                    @endif
              @else
