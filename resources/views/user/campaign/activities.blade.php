    ...
    <div class="content">
        ...
        <ol class="list-group list-group-numbered mb-3">
            ...
            <li class="list-group-item d-flex justify-content-between align-items-start">
                ...
                <span class="badge bg-primary rounded-pill">{{ baseCurrency() }}
                    {{ number_format(@$campaignStat['counts']['Approved'] * $amount) }} / {{ baseCurrency() }}
                    {{ number_format($amount * $lists->number_of_staff) }}</span>
            </li>
            ...
            <li class="list-group-item d-flex justify-content-between align-items-start">
                ...
                <span class="badge bg-primary rounded-pill">
                    {{ @$campaignStat['is_completed'] ? 'Completed' : 'Not Completed' }} </span>
            </li>
                            ...
                            @foreach ($responses as $list)
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        {{ baseCurrency() }}
                                        {{ currencyConverter($list->currency, baseCurrency(), $list->amount)  }}
                                    </td>
                                    <td>{{ $list->status }}</td>
                                    <td>
                                        @if($list->status == 'Pending')
                                            ...
                                            @if($lists->completed()->where('status', 'Approved')->count() >= @$list->campaign->number_of_staff)
                                                <button type="button" class="btn btn-alt-warning btn-sm disabled">Worker
                                                    Completed</button>
                                            @else

                                                <a href="{{ url('campaign/activities/' . $list->id . '/response') }}"
                                                    class="btn btn-alt-primary btn-sm"> View Response </a>
                                                {{-- <button type="button" class="btn btn-alt-primary btn-sm" data-bs-toggle="modal"
                                                    data-bs-target="#modal-default-popout-{{ $list->id }}">View</button> --}}
                                            @endif
