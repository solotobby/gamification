<div class="content-header">
    ...
    <div class="space-x-1">
        ...
        <!-- Open Search Section -->
        <!-- Layout API, functionality initialized in Template._uiApiLayout() -->
        @if(auth()->user()->role == 'admin' || auth()->user()->role == 'staff' || auth()->user()->role == 'super_admin')
        ...
        <strong>Welcome {{ short_name(auth()->user()->name) }}
            @php
                $user = auth()->user();
                $isNaira = ($user?->wallet?->base_currency === 'NGN') || $user?->hasRole('super_admin');
            @endphp
    ...
    <div class="space-x-1">
        ...
        <div class="dropdown d-inline-block">
            <button type="button" class="btn btn-alt-secondary" id="page-header-notifications-dropdown"
                ...
                <span><small>{{ auth()->user()->notifications()->where('is_read', false)->count() }}</small></span>
            </button>
            <div class="dropdown-menu dropdown-menu-lg dropdown-menu-end p-0"
                ...
                <ul class="nav-items my-2" id="notifications"
                    style="max-height: 300px; overflow-y: scroll; border: 1px solid #ccc; padding: 10px; list-style-type: none;">
        ...
        <div class="content-header">
            <form class="w-100" action="{{ url('users/search') }}" method="GET">
                <div class="input-group">
                    ...
                    <input type="text" name="q" class="form-control border-0" placeholder="Search or hit ESC.."
